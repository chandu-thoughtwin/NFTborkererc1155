...
contract Borker   {
   ERC1155MixedFungibleMintable ercmixed1155;
    ...
    mapping(uint256 => acuction) acuctions;

    mapping(uint256 => bool) public NFTforSale ;
    
    

    function putOnSell(address _tokenOwner,  uint256 _buyPrice , uint256 nfttokenID) public {
        
        if (ercmixed1155.ownerOf(nfttokenID)== msg.sender){
        require( ercmixed1155.isApprovedForAll(_tokenOwner,address(this)),"not approved");
        require(ercmixed1155.isNonFungible(nfttokenID),"Token is not NFT");
        require(NFTforSale[nfttokenID] == false," Token alreay on sale ");

        acuction memory newAuction = acuction(ercmixed1155.ownerOf(nfttokenID),_buyPrice);

        acuctions[nfttokenID] = newAuction;
     ...
     function buyNFT(uint256 nfttokenID, uint256 amount, uint256 _ftID) public{
         
        acuction memory _acuction = acuctions[nfttokenID];
         
         if(NFTforSale[nfttokenID]== true && ercmixed1155.isFungible(_ftID)==true) {

             require(amount >= _acuction.buyPrice,"not enough amount");
             
             require(ercmixed1155.isApprovedForAll(msg.sender,address(this)));
             
            //  transferNFT
        
        require(ercmixed1155.balanceOf(msg.sender,_ftID)>=_acuction.buyPrice && amount >= _acuction.buyPrice);
        ierc1155.safeTransferFrom(msg.sender,_acuction.tokenOwner,_ftID,amount,"");
        ierc1155.safeTransferFrom(_acuction.tokenOwner,msg.sender,nfttokenID,0,"");
         }
